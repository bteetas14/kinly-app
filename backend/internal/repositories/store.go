...
func (s *Store) ListProducts(ctx context.Context, filters dto.ProductFilters, limit, offset int) ([]models.Product, int64, error) {
	...
	query := `
		SELECT p.id, p.brand_id, b.name, p.category_id, c.name, coalesce(p.subcategory_id::text, ''), coalesce(sc.name, ''),
		       ...
		       coalesce(avg(r.rating), 0)::float, coalesce(avg(CASE WHEN r.repurchased THEN 1.0 ELSE 0.0 END), 0)::float,
...
func (s *Store) CreateReview(ctx context.Context, userID string, req dto.CreateReviewRequest) (models.Review, error) {
	...
	err = tx.QueryRowContext(ctx, `
		INSERT INTO reviews (product_id, user_id, rating, title, body, pros, cons, would_buy_again, repurchased, verified_purchase)
		VALUES ($1, $2, $3, $4, $5, $6::text[], $7::text[], $8, $9, $10)
		RETURNING id, product_id, user_id, rating, title, body, array_to_json(pros), array_to_json(cons), would_buy_again, repurchased, verified_purchase, helpful_count, created_at
	`, req.ProductID, userID, req.Rating, req.Title, req.Body, textArray(req.Pros), textArray(req.Cons), req.WouldBuyAgain, req.Repurchased, req.VerifiedPurchase).Scan(
		...
		&review.VerifiedPurchase, &review.HelpfulCount, &review.CreatedAt,
	)
	if err != nil {
		return models.Review{}, err
	}
...
func (s *Store) ListReviews(ctx context.Context, productID, sort string, limit, offset int) ([]models.Review, int64, error) {
	...
	rows, err := s.db.QueryContext(ctx, `
		SELECT r.id, r.product_id, r.user_id, u.username, u.trust_score, r.rating, r.title, r.body,
		       array_to_json(r.pros), array_to_json(r.cons), r.would_buy_again, r.repurchased, r.verified_purchase,
		       r.helpful_count, coalesce((SELECT json_agg(image_url) FROM review_photos rp WHERE rp.review_id = r.id), '[]'::json), r.created_at
		FROM reviews r
...
func productOrder(sort string) string {
	...
	case "most_trusted_reviews":
		return " ORDER BY coalesce(avg((SELECT trust_score FROM users WHERE users.id = r.user_id)), 0) DESC"
	...
	case "highest_rated":
		return " ORDER BY coalesce(avg(r.rating), 0) DESC"
	case "trending":
		fallthrough
	default:
...
func reviewOrder(sort string) string {
	...
	case "highest_trust_reviewer":
		return " ORDER BY u.trust_score DESC, r.created_at DESC"
	case "newest":
...
func scanProducts(rows *sql.Rows) ([]models.Product, int64, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&p.SensitiveSkin, &p.CrueltyFree, &p.FragranceFree, &p.Vegan, &p.CommunityScore, &p.RepurchaseRate,
			&p.ReviewCount, &p.TrendingScore, &p.CreatedAt,
		); err != nil {
			return nil, 0, err
		}
...
func scanReviews(rows *sql.Rows) ([]models.Review, error) {
	...
	for rows.Next() {
		var r models.Review
		if err := rows.Scan(
			&r.ID, &r.ProductID, &r.UserID, &r.Username, &r.UserTrustScore, &r.Rating, &r.Title, &r.Body,
			jsonScanner(&r.Pros), jsonScanner(&r.Cons), &r.WouldBuyAgain, &r.Repurchased, &r.VerifiedPurchase,
			&r.HelpfulCount, jsonScanner(&r.Photos), &r.CreatedAt,
		); err != nil {
			return nil, err
		}
